...
### Added (2026-06-08, follow-up)
- **Prompt Inspector panel: User Prompt tab** (5-module structure view). The panel's tab bar now includes a new "User prompt (5 modules)" tab that renders the 5-module user prompt the LLM would see, with collapsible sub-sections: rendered prompt, section breakdown (10 fields), and template placeholders. Auto-hides if the dev flag is off (same `v-if="enabled"` guard as the rest of the panel).
- **End-to-end tests for the 5-module prompt + choices pipeline** (`backend/tests/test_5module_e2e.py`): 17 tests across 4 test classes covering the 5-module prompt structure, `_validate_and_extract_choices` (5 sub-cases including the soft numeric-content check), MockLLMClient choices passthrough (4 sub-cases including default canned, custom, missing, and non-dict entries), and ActionProcessor `process()` end-to-end flow (4 sub-cases including the captured-user-message test that verifies the LLM actually receives the 5-module structure).
- **MockLLMClient default canned response is now 5-module compliant** (`backend/llm_client.py`): the default canned JSON includes a Chinese narrative, a `null` state_mutations (no F3 contract violation), and 4 well-formed choices (one of each direction). The mock's `generate_with_state_contract` now surfaces the LLM's `choices` array in the result dict (previously dropped), so the action processor's `_validate_and_extract_choices` helper can pick them up.
- **Refined numeric soft-check** in `_validate_and_extract_choices` (`backend/api/action_processor.py`): now character-level (was token-level). Drops vignettes whose digit ratio exceeds 50% **and** whose length is >= 5 chars. Short tags like "v1" or "hp" are preserved; long numeric-heavy strings like "1234567890abcd" are dropped. Aligns with Module 5 "Risks ... 不可提供數字".
- **Prompt Inspector test coverage extended** (`backend/tests/test_prompt_inspector.py`): 1 new test (`test_preview_user_prompt_section`) verifies the `/preview` endpoint returns the full `user_prompt` block with 5-module structure, allowed directions, section breakdown, and template constants.
