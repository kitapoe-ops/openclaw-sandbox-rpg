...
class TestEquipmentConstraints:
    ...
    def test_equipment_constraints_fallback(
        self, no_palace_builder, basic_state, basic_action_context
    ):
    ...
    def test_equipment_constraints_no_equipped(
        self, no_palace_builder, basic_state, basic_action_context
    ):
    ...
    def test_equipment_constraints_injection(self, no_palace_builder, basic_action_context):
        ...
        assert (
            "裝備：【勇者之劍】（特性：鋒利, 神聖傷害, 沉重, 導電）" not in prompt
        )  # HIDDEN 2026-06-08
        ...
        assert (
            "1. 生成戰鬥或破壞類選項時，必須優先考慮使用【勇者之劍】。" not in prompt
        )  # HIDDEN 2026-06-08
